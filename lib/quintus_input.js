...
Quintus.Input = function(Q) {
  ...
  Q.component("platformerControls", {
    defaults: {
      speed: 100, //VELOCIDAD DEL PERSONAJE
      jumpSpeed: -300,
      collisions: []
    },
    ...
    step: function(dt) {
      ...
      if(p.ignoreControls === undefined || !p.ignoreControls) {
        ...
        if(p.landed > 0 && (Q.inputs['Q'] || Q.inputs['action']) && !p.jumping) {
          p.vy = p.jumpSpeed;
          p.landed = -dt;
          p.jumping = true;
        } else if(Q.inputs['Q'] || Q.inputs['action']) {
          this.entity.trigger('jump', this.entity);
          p.jumping = true;
        }

        if(p.jumping && !(Q.inputs['Q'] || Q.inputs['action'])) {
          p.jumping = false;
